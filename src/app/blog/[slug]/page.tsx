...
export async function generateMetadata({ params }: any): Promise<Metadata> {
  const post = await fetchSanity<Post>(postBySlugQuery, { slug: params.slug, locale: "da" });
  if (!post) return {};
...
export default async function PostPage({ params }: any) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 space-y-6">
      <JsonLd id="breadcrumbs" data={jsonLd.breadcrumb([{ name: "Forside", item: base }, { name: "Blog", item: `${base}/blog` }, { name: post.title, item: url }])} />
      <JsonLd
        id="article"
        data={jsonLd.article({
          headline: post.title,
